...
function detectCategory(text: string): CalendarEvent['category'] {
  ...
  if (/\bclases?\b/i.test(text) || /\basignaturas?\b/i.test(text) || /\bcursos?\b/i.test(text) || ((/inscrip/i.test(tl) && /mater/i.test(tl)) || /cursos? de verano/i.test(tl) || /cursos? intensiv/i.test(tl) || /receso/i.test(tl))) {
    return 'clases';
  }
